...
- [ ] 1.1 `themes/terminal/layouts/_partials/head.html`: add Open Graph and
...
- [ ] 1.2 Resolve each value from the page's own front matter with a fallback to
...
- [ ] 1.3 Add a default share image to `static/` and reference it as an absolute
  URL. A card with no image is a thin grey strip on most platforms.
- [ ] 1.4 Do not mark any of these values `| safe`. Titles and descriptions are
  free text.
- [ ] 1.5 `partialCached` is used for the css/js partials in this file — do not
  extend caching to the OG block, whose values vary per page.

## 2. Share page template

- [ ] 2.1 Have the Hugo build emit a share-page template into the web root — a
...
- [ ] 2.2 Give the template a path that is clearly not a visitable page and that
  the refresher can find deterministically.
- [ ] 2.3 Token set covers: canonical URL, title, plain-text description, image
...
- [ ] 2.4 The template's event body links to `/calendar/` and the announcement
  body to `/announcements/` for onward browsing. This is a link, not a redirect.

## 3. On-request generation

- [ ] 3.1 A small responder bound to loopback only, started by systemd, that
...
- [ ] 3.2 Caddy: serve the generated file when it exists and fall through to the
  responder when it does not, for the two share path prefixes only. Nothing else
  on the site changes routing.
- [ ] 3.3 One renderer. The responder and the refresher MUST call the same fill
...
- [ ] 3.4 An id that does not resolve to a public item returns not-found and
  ...
  the reconciler.
- [ ] 3.5 The responder is not reachable from outside the host. It takes an id
...
- [ ] 4.1 New `deploy/share-pages/` holding a refresh script, a oneshot
...
- [ ] 4.2 Fetch public events over a bounded window using the API's `from`/`to`
...
- [ ] 4.3 Fetch public announcements from `/public/announcements`.
- [ ] 4.4 Select events by `visibility === "Public"`. Members-only entries arrive
...
- [ ] 4.5 Write `/e/<id>/index.html` and `/a/<id>/index.html` under the web root
  by filling the template.
- [ ] 4.6 Maintain a crawlable index of the generated pages — a sitemap the
...
- [ ] 4.7 Resolve `image_url` against the API origin. The API returns paths like
  `uploads/<uuid>.jpg`, relative to Coterie, not to this site.
- [ ] 4.8 Reconcile, do not merely add — but scope removal to what was actually
...
- [ ] 4.9 Maintain a manifest the refresher owns, recording each generated page's
...
- [ ] 4.10 Age-out governs polling only. Stop refreshing an item older than the
...
- [ ] 4.11 Make reconciliation conditional on a successful fetch of **both**
...
- [ ] 4.12 Never delete anything outside the generated path prefixes. Compute the
  delete set from directories the refresher itself owns, not from a glob of the
  web root.
- [ ] 4.13 Escape every interpolated value for its context. `content_html` from
...
- [ ] 4.14 Derive the metadata description as plain text — strip markup, bound
  the length on a word boundary. Do not put `content_html` in an `og:description`.

## 5. Retraction

- [ ] 5.1 Pick the timer interval as an exposure bound and say so in the timer
...
- [ ] 5.2 Document the immediate-purge action in the README next to the install
...
- [ ] 5.3 Serve share pages with `no-cache` plus an ETag in the Caddy block —
...
- [ ] 5.4 Do NOT fingerprint or version the share URL. Asset fingerprinting works
...
- [ ] 5.5 Confirm by test that the on-request path cannot resurrect a removed
...
- [ ] 5.6 Do not claim in docs that removal recalls anything already scraped. A
...
- [ ] 6.1 `deploy.sh` line 7 is `rsync -av --delete --exclude=yt-feed.xml`. Add
  excludes for the generated share-page paths. **Without this the next deploy
  deletes every generated page.**
- [ ] 6.2 Fix `deploy/yt-feed-cache/README.md:35`, which states "deploys rsync
...
- [ ] 6.3 State the exclude requirement in the new README too, next to the
  install steps, so the two generated paths are documented in the same place they
  are created.

## 7. Copy control

- [ ] 7.1 In the event modal, the copy control copies `/e/<id>/` as an absolute
  URL. The `#event-<id>` fragment and its month parameter stay exactly as they
  are for in-page state and `hashchange`.
- [ ] 7.2 Keep one copy-and-confirm helper shared with the announcement modal;
  the announcement control now copies `/a/<id>/`.
- [ ] 7.3 Both copy controls fire a request to the share URL when used, priming
...
- [ ] 8.1 The rendered `head` carries OG and Twitter tags with page-specific
  values, and falls back to site values on a page with no front-matter
  description.
- [ ] 8.2 `og:url` and `og:image` are absolute.
- [ ] 8.3 Refresher: a payload with two public events and one members-only event
  produces exactly two event pages.
- [ ] 8.4 An id present on disk and inside the queried window, but absent from the
  payload, has its directory removed.
- [ ] 8.5 An id present on disk whose item falls **outside** the queried window is
  ...
  window arithmetic.
- [ ] 8.6 A page whose item is older than the refresh window is kept and not
  re-fetched. Assert it is still present after a run — deleting for age is the
  regression this guards.
- [ ] 8.7 On-request: a public item with no page on disk is generated and returned;
  the file exists afterwards and a second request does not regenerate it.
- [ ] 8.8 On-request: an id that is not a public item returns not-found and leaves
  no file behind — assert the absence, since a stray placeholder is what would
  confuse the reconciler.
- [ ] 8.9 On-request and refresher output for the same item are byte-identical.
  This is the assertion that keeps the two paths from drifting.
- [ ] 8.10 A responder request with a path segment that is not a well-formed id —
  including traversal attempts — is rejected before any filesystem access.
- [ ] 8.11 Using a copy control for an item with no page on disk results in that
  page existing, and the clipboard write is not blocked by the prime request.
- [ ] 8.12 Retraction: a page exists, its item flips to members-only, reconcile
...
- [ ] 8.13 A failed fetch — non-2xx, and separately an unparseable body — leaves
...
- [ ] 8.14 A title containing `"`, `<`, and `&` cannot break out of either the
  metadata attribute or the page body.
- [ ] 8.15 An item with no image produces no item-specific `og:image`.
- [ ] 8.16 The refresher deletes nothing outside its own path prefixes — give it a
  web root containing an unrelated file and assert the file survives.
- [ ] 8.17 The copy control yields the share page URL, not the calendar deep link.
- [ ] 8.18 Announcement deep links still work: `#announcement-<id>` opens and
  clears as before. The shared copy helper is being changed underneath them.

## 9. Build

- [ ] 9.1 Rebuild `public/` (`hugo --gc --minify`) so the committed output
