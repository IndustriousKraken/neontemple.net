...
- [x] 1.1 Add the redirects to `deploy/caddy/theneontemple.com.caddy`, which is
...
- [x] 1.2 Use permanent (301) redirects so search engines transfer the indexed
  URL rather than continuing to offer a failing link.
- [x] 1.3 Map, at minimum, the paths observed in the access log:
  ...
  - `/feed/`, `/feed/atom/`, `/rss/`
  - `/login`
- [x] 1.4 Choose each destination by what the legacy page was for, not by name
...
- [x] 1.5 Where no honest equivalent exists, redirect to the nearest true
  destination rather than inventing a page. Do not create stub pages to be
  redirect targets.
- [x] 1.6 Place the redirects so they are evaluated before the static file
...
- [x] 2.1 `/events/ics/` redirects to the iCal feed Coterie serves at
  `/public/feed/calendar` on the API origin.
- [x] 2.2 Verify a calendar client following the redirect actually receives
...
- [x] 2.3 Check whether the redirect needs to preserve any query the legacy feed
  accepted, and if it does not, say so in a comment rather than leaving it
  unconsidered.

## 3. No catch-all

- [x] 3.1 Do NOT add a fallback redirect for unmatched paths. Unmatched requests
  keep returning 404.
- [x] 3.2 Record the reason beside the map: the bulk of 404 traffic is hostile
...
- [x] 3.3 Keep the `neontemple.net` block's path-preserving redirect exactly as
  it is. It is why per-path mapping is possible.

## 4. Verification

- [x] 4.1 Each mapped path returns 301 to its intended destination, and that
  destination returns 200.
- [x] 4.2 `/events/ics/` yields calendar data at the end of the redirect chain.
- [x] 4.3 A path from the legacy hostname — `neontemple.net/events/` — completes
  the whole chain to the calendar, since that is how these requests actually
  arrive.
- [x] 4.4 An unmapped path still returns 404. Assert this explicitly; it is the
  property task 3.1 protects and the easiest to lose by adding one convenient
  rule later.
- [x] 4.5 Existing routes are unaffected: `/`, `/calendar/`, `/announcements/`,
  `/join/`, `/yt-feed`, and a generated share page under `/e/`.

## 5. Documentation

- [x] 5.1 Note in the deploy README how the map was derived — filtering 404s in
