...
- [x] 1.1 Create `themes/terminal/assets/js/api.test.js` that loads `api.js` in a
...
- [x] 1.2 `fetch_rejects_with_backend_message_on_non_ok_response` — stub `fetch`
  ...
  `'boom'`.
- [x] 1.3 `fetch_falls_back_to_http_status_when_body_unparseable` — stub `fetch`
...
- [x] 1.4 `fetch_propagates_network_failure` — stub `fetch` to reject with
  `new TypeError('network down')` and assert `CoterieAPI.fetch('/x')` rejects with
  that same error.
- [x] 1.5 `fetch_returns_parsed_json_on_ok_response` — stub `fetch` to resolve
...
- [x] 2.1 `get_events_combines_limit_and_type_params` — stub `fetch` to capture
  the requested endpoint and assert `getEvents({ limit: 25, type: 'ctf' })`
  requests `/public/events?limit=25&type=ctf`.
- [x] 2.2 `get_events_requests_bare_endpoint_without_params` — assert
  `getEvents()` requests `/public/events` with no query string.
- [x] 2.3 `get_events_omits_zero_limit` — assert `getEvents({ limit: 0 })`
  requests `/public/events` (the falsy `0` is not appended as a `limit`).
- [x] 2.4 `get_announcements_appends_only_supplied_params` — assert
