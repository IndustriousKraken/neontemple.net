...
- [x] 1.1 `getImageUrl_returns_absolute_url_unchanged` — load `main.js` via the
...
- [x] 1.2 `getImageUrl_joins_relative_path_to_configured_base` — after
...
- [x] 1.3 `getImageUrl_roots_relative_path_when_base_unset` — with no
  `window.COTERIE_API_URL` set on the sandbox, assert `getImageUrl('uploads/a.jpg')`
  returns `'/uploads/a.jpg'`.
- [x] 1.4 `getImageUrl_returns_empty_string_for_falsy_input` — assert
