...
- [x] 1.1 `hugo.toml`: add `turnstileSiteKey = ''` (public site key; empty = off).
- [x] 1.2 `themes/terminal/layouts/_default/join.html`: when `turnstileSiteKey` is
...
- [x] 1.3 `themes/terminal/assets/js/main.js` `initSignupForm`: read
  `cf-turnstile-response` from the form data, send it as `captcha_token` (omit
  when absent), and `turnstile.reset()` on a failed submit.

## 2. Tests

- [x] 2.1 Build with `turnstileSiteKey` set → `join/index.html` contains the
  Turnstile script and a `cf-turnstile` element with the key; build with it empty
  → neither is present.
- [x] 2.2 `initSignupForm` includes `captcha_token` in the request when a
  `cf-turnstile-response` value is present, and omits it otherwise.

## 3. Verify

- [x] 3.1 `openspec validate join-form-turnstile-widget --strict` passes.
- [x] 3.2 `npm test` green; `hugo` builds (both key states).

## 4. Rollout (operator)

<!-- 4.1/4.2 are live operator steps (deploy + Coterie backend config/restart);
     they run outside the executor sandbox and are left for the operator. -->
