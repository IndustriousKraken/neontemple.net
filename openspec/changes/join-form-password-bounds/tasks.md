...
- [ ] 1.1 `themes/terminal/layouts/_default/join.html`: add a visible hint to the
...
- [ ] 1.2 Extend the existing comment above the field (it currently explains only
...
- [ ] 1.3 Keep `minlength="10"`. Both attributes count UTF-16 code units rather
...
- [ ] 1.4 Do NOT add `maxlength`.

## 2. Optional client feedback

- [ ] 2.1 If adding a live warning in `themes/terminal/assets/js/main.js`, measure
...
- [ ] 2.2 The warning SHALL NOT modify, clip, or block the field, and SHALL NOT
  prevent submission. The backend's rejection message stays authoritative.

## 3. Verify

- [ ] 3.1 `hugo --gc --minify` builds clean.
- [ ] 3.2 The hint renders on the join page and reads sensibly at mobile width —
  it sits next to a masked field, so it must not wrap into noise.
- [ ] 3.3 Pasting a 200-character password leaves all 200 characters in the field.
  This is the regression that matters: it fails the moment someone adds
  `maxlength`.
- [ ] 3.4 `openspec validate join-form-password-bounds --strict` passes.
