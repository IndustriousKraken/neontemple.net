...
- [ ] 1.1 Add a `safeRegistrationUrl(value)` helper in
...
- [ ] 1.2 Fail closed and quietly: no console output carrying the rejected value.

## 2. Price formatting

- [ ] 2.1 Add a small formatter turning `guest_price_cents` into display text:
...
- [ ] 3.1 In `renderEventCard`, when `safeRegistrationUrl(event.registration_url)`
  ...
  events.
- [ ] 3.2 Route the badge text through the existing escaping helpers, per
  `content-rendering`.
- [ ] 3.3 Do NOT make the badge clickable; the card's existing `onclick` stays the
  only click target.

## 4. Modal button

- [ ] 4.1 In `showEventModal`, render an anchor to the validated URL with a label
  naming the action and the cost. Omit the whole block when the URL is absent or
  rejected.
- [ ] 4.2 Give it `rel="noopener"` if it opens in a new tab; decide one behavior
  and apply it consistently.
- [ ] 4.3 Clear the block on every modal open so a previously-viewed registerable
...
- [ ] 5.1 Badge and button styles in the theme's CSS, matching existing card and
  button conventions rather than introducing a new visual language.
- [ ] 5.2 Verify the badge reads correctly in both the calendar grid and the
  home-page upcoming list, since `renderEventCard` serves both.

## 6. Tests / verification

- [ ] 6.1 An event with no `registration_url` renders no badge and no button.
- [ ] 6.2 A `javascript:` URL renders neither, and inserts no anchor.
- [ ] 6.3 A zero-price registerable event reads as free, not `$0.00`.
- [ ] 6.4 Opening a registerable event's modal then a non-registerable one leaves
  no stale button.
- [ ] 6.5 Build with `hugo --gc --minify` and confirm the calendar and home page
