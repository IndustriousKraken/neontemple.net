...
function showEventModal(eventId) {
  ...
  // Reflect the open event in the URL so it can be linked/shared directly, and
  // bind the copy handler to a closure over that link. The id is never
  ...
  if (link) {
    ...
    if (btn) btn.onclick = () => copyLink(btn, link);
...
function copyAnnouncementLink(btn) {
  ...
  copyLink(btn, m ? location.origin + '/announcements/#announcement-' + m[1] : location.href);
}
